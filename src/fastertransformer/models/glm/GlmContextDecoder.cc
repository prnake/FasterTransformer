...
template<typename T>
void GlmContextDecoder<T>::initialize()
{
    ...
    glu_ffn_layer_ = new TensorParallelGeluGluFfnLayer<T>(max_batch_size_,
                                                   max_seq_len_,
                                                   head_num_,
                                                   size_per_head_,
                                                   inter_size_,
...
template<typename T>
bool GlmContextDecoder<T>::isValidBatchSize(size_t batch_size)
{
...
template<typename T>
bool GlmContextDecoder<T>::isValidSeqLen(size_t seq_len)
{
...
template<typename T>
bool GlmContextDecoder<T>::isValidLayerParallelId(uint l)
{
...
template<typename T>
bool GlmContextDecoder<T>::isFirstLayerParallelId(uint l)
{
    int local_num_layer = (int)(ceil(num_layer_ * 1.0f / pipeline_para_.world_size_));
    return l < num_layer_ && (l == local_num_layer * pipeline_para_.rank_);
}
...
template<typename T>
bool GlmContextDecoder<T>::isLastLayerParallelId(uint l)
{
    int local_num_layer = (int)(ceil(num_layer_ * 1.0f / pipeline_para_.world_size_));
...
template<typename T>
int GlmContextDecoder<T>::getFirstLayerParallelId()
{
    int local_num_layer = (int)(ceil(num_layer_ * 1.0f / pipeline_para_.world_size_));
...
template<typename T>
GlmContextDecoder<T>::GlmContextDecoder(size_t max_batch_size,
                                          size_t max_seq_len,
                                          size_t head_num,
...
template<typename T>
GlmContextDecoder<T>::GlmContextDecoder(GlmContextDecoder<T> const& decoder):
...
template<typename T>
GlmContextDecoder<T>::~GlmContextDecoder()
{
    delete self_attention_layer_;
    delete glu_ffn_layer_;
    freeBuffer();
}

template<typename T>
void GlmContextDecoder<T>::forward(std::vector<Tensor>* output_tensors,
                                    const std::vector<Tensor>* input_tensors,
                                    const std::vector<GlmDecoderLayerWeight<T>>* gpt_decoder_layer_weight)
{
...
template<typename T>
void GlmContextDecoder<T>::forward(std::unordered_map<std::string, Tensor>* output_tensors,
...
template class GlmContextDecoder<float>;
template class GlmContextDecoder<half>;
