...
struct GlmWeight {
    ...
    GlmWeight() = default;
    GlmWeight(const int hidden_units,
    ...
    ~GlmWeight();
    GlmWeight(const GlmWeight& other);
    GlmWeight& operator=(const GlmWeight& other);

    void loadModel(std::string dir_path);

    std::vector<GlmDecoderLayerWeight<T>> decoder_layer_weights;
    const T* pre_decoder_embedding_table = nullptr;
    // GPT-J does not use embedding table, but we leave the ptr such that
    // Glm::forward and Gpt::forward become identical
