...
template<typename T>
bool GlmDecoder<T>::isValidBatchSize(size_t batch_size)
{
...
template<typename T>
bool GlmDecoder<T>::isValidLayerParallelId(uint l)
{
...
template<typename T>
bool GlmDecoder<T>::isFirstLayerParallelId(uint l)
{
    int local_num_layer = (int)(ceil(num_layer_ * 1.0f / pipeline_para_.world_size_));
    return l < num_layer_ && (l == local_num_layer * pipeline_para_.rank_);
}
...
template<typename T>
bool GlmDecoder<T>::isLastLayerParallelId(uint l)
{
    int local_num_layer = (int)(ceil(num_layer_ * 1.0f / pipeline_para_.world_size_));
...
template<typename T>
int GlmDecoder<T>::getFirstLayerParallelId()
{
    int local_num_layer = (int)(ceil(num_layer_ * 1.0f / pipeline_para_.world_size_));
...
template<typename T>
GlmDecoder<T>::GlmDecoder(size_t max_batch_size,
                            size_t head_num,
...
template<typename T>
GlmDecoder<T>::GlmDecoder(GlmDecoder<T> const& decoder):
...
template<typename T>
void GlmDecoder<T>::forward(std::vector<Tensor>* output_tensors,
...
template<typename T>
void GlmDecoder<T>::forward(std::unordered_map<std::string, Tensor>* output_tensors,
...
template class GlmDecoder<float>;
template class GlmDecoder<half>;
