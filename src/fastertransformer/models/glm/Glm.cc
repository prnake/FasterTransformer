...
{
    gpt_context_decoder_ = new GlmContextDecoder<T>(0,
                                                     0,
                                                     head_num_,
                                                     size_per_head_,
                                                     inter_size_,
    ...
    gpt_decoder_ = new GlmDecoder<T>(0,
                                      head_num_,
                                      size_per_head_,
                                      inter_size_,
                                      num_layer_,
...
template<typename T>
void Glm<T>::allocateBuffer()
{
    FT_CHECK(false);
}

template<typename T>
void Glm<T>::allocateBuffer(size_t batch_size, size_t beam_width, size_t max_seq_len, size_t max_input_len)
{
...
template<typename T>
size_t Glm<T>::getPipelineParallelRank()
{
    return pipeline_para_.rank_;
}

template<typename T>
size_t Glm<T>::getPipelineParallelSize()
{
    return pipeline_para_.world_size_;
}

template<typename T>
size_t Glm<T>::getTensorParallelRank()
{
    return tensor_para_.rank_;
}

template<typename T>
size_t Glm<T>::getTensorParallelSize()
{
    return tensor_para_.world_size_;
}

template<typename T>
bool* Glm<T>::getFinishBuffer()
{
    return finished_buf_;
}

template class Glm<float>;
template class Glm<half>;
