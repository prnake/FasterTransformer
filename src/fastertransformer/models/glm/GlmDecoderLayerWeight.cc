    ...
    tensor_para_rank_(other.tensor_para_rank_)
{
    ...
    cudaD2Dcpy(weights_ptr[7], other.weights_ptr[7], hidden_units_ * inter_size_ / tensor_para_size_);
    cudaD2Dcpy(weights_ptr[8], other.weights_ptr[8], inter_size_ / tensor_para_size_);
    cudaD2Dcpy(weights_ptr[9], other.weights_ptr[9], inter_size_ / tensor_para_size_ * hidden_units_);
...
void GlmDecoderLayerWeight<T>::loadModel(std::string dir_path, FtCudaDataType model_file_type)
{
    ...
    loadWeightFromBin<T>(weights_ptr[5],
                         ...
                         dir_path + ".mlp.dense_h_to_4h_1.weight." + rank_spec + ".bin",
                         model_file_type);
    loadWeightFromBin<T>(weights_ptr[6],
                         {inter_size_ / tensor_para_size_},
                         dir_path + ".mlp.dense_h_to_4h_1.bias." + rank_spec + ".bin",
                         model_file_type);
    loadWeightFromBin<T>(weights_ptr[7],
                         {hidden_units_, inter_size_ / tensor_para_size_},
                         dir_path + ".mlp.dense_h_to_4h_2.weight." + rank_spec + ".bin",
                         model_file_type);
    loadWeightFromBin<T>(weights_ptr[8],
                         {inter_size_ / tensor_para_size_},
                         dir_path + ".mlp.dense_h_to_4h_2.bias." + rank_spec + ".bin",
                         model_file_type);
    loadWeightFromBin<T>(weights_ptr[9],
                         ...
                         model_file_type);
    loadWeightFromBin<T>(weights_ptr[10], {hidden_units_}, dir_path + ".mlp.dense_4h_to_h.bias.bin", model_file_type);
}
...
void GlmDecoderLayerWeight<T>::setWeightPtr()
{
    ...
    glu_ffn_weights.intermediate_weight[0].kernel = weights_ptr[5];
    glu_ffn_weights.intermediate_weight[0].bias = weights_ptr[6];
    glu_ffn_weights.intermediate_weight[1].kernel = weights_ptr[7];
    glu_ffn_weights.intermediate_weight[1].bias = weights_ptr[8];
    glu_ffn_weights.output_weight.kernel = weights_ptr[9];
    glu_ffn_weights.output_weight.bias = weights_ptr[10];
...
void GlmDecoderLayerWeight<T>::mallocWeights()
{
    ...
    deviceMalloc(&weights_ptr[7], hidden_units_ * inter_size_ / tensor_para_size_);
    deviceMalloc(&weights_ptr[8], inter_size_ / tensor_para_size_);
    deviceMalloc(&weights_ptr[9], inter_size_ / tensor_para_size_ * hidden_units_);
