...
GlmDecoderLayerWeight<T>& GlmDecoderLayerWeight<T>::operator=(const GlmDecoderLayerWeight& other)
{
    ...
    cudaD2Dcpy(weights_ptr[0], other.weights_ptr[0], hidden_units_ * 3 * hidden_units_ / tensor_para_size_);
    cudaD2Dcpy(weights_ptr[1], other.weights_ptr[1], 3 * hidden_units_ / tensor_para_size_);
    cudaD2Dcpy(weights_ptr[2], other.weights_ptr[2], hidden_units_ * hidden_units_ / tensor_para_size_);
    cudaD2Dcpy(weights_ptr[3], other.weights_ptr[3], hidden_units_);
    cudaD2Dcpy(weights_ptr[4], other.weights_ptr[4], hidden_units_);
    cudaD2Dcpy(weights_ptr[5], other.weights_ptr[5], hidden_units_);
    cudaD2Dcpy(weights_ptr[6], other.weights_ptr[6], hidden_units_ * inter_size_ / tensor_para_size_);
...
void GlmDecoderLayerWeight<T>::loadModel(std::string dir_path, FtCudaDataType model_file_type)
{
    ...
    loadWeightFromBin<T>(weights_ptr[2],
                         {hidden_units_, hidden_units_ / tensor_para_size_},
                         dir_path + ".attention_output_weight.kernel." + rank_spec + ".bin",
                         model_file_type);
    loadWeightFromBin<T>(weights_ptr[3],
                         {hidden_units_},
                         dir_path + ".attention_output_weight.bias." + rank_spec + ".bin",
                         model_file_type);
    loadWeightFromBin<T>(weights_ptr[4],
                         {hidden_units_},
                         dir_path + ".self_attn_layernorm_weights.beta." + rank_spec + ".bin",
                         model_file_type);
    loadWeightFromBin<T>(weights_ptr[5],
                         {hidden_units_},
                         dir_path + ".self_attn_layernorm_weights.gamma." + rank_spec + ".bin",
                         model_file_type);
    loadWeightFromBin<T>(weights_ptr[6],
                         {hidden_units_, inter_size_ / tensor_para_size_},
                         dir_path + ".glu_ffn_weights.intermediate_weight_1.kernel." + rank_spec + ".bin",
                         model_file_type);
    loadWeightFromBin<T>(weights_ptr[7],
                         {inter_size_ / tensor_para_size_},
                         dir_path + ".glu_ffn_weights.intermediate_weight_1.bias." + rank_spec + ".bin",
                         model_file_type);
    loadWeightFromBin<T>(weights_ptr[8],
                         {hidden_units_, inter_size_ / tensor_para_size_},
                         dir_path + ".glu_ffn_weights.intermediate_weight_2.kernel." + rank_spec + ".bin",
                         model_file_type);
    loadWeightFromBin<T>(weights_ptr[9],
                         {inter_size_ / tensor_para_size_},
                         dir_path + ".glu_ffn_weights.intermediate_weight_2.bias." + rank_spec + ".bin",
                         model_file_type);
    loadWeightFromBin<T>(weights_ptr[10],
                         {inter_size_ / tensor_para_size_, hidden_units_},
                         dir_path + ".glu_ffn_weights.output_weight.kernel." + rank_spec + ".bin",
                         model_file_type);
    loadWeightFromBin<T>(weights_ptr[11],
                         {hidden_units_},
                         dir_path + ".glu_ffn_weights.output_weight.bias." + rank_spec + ".bin",
                         model_file_type);
    loadWeightFromBin<T>(weights_ptr[12],
                         {hidden_units_},
                         dir_path + ".glu_ffn_layernorm_weights.beta." + rank_spec + ".bin",
                         model_file_type);
    loadWeightFromBin<T>(weights_ptr[13],
                         {hidden_units_},
                         dir_path + ".glu_ffn_layernorm_weights.gamma." + rank_spec + ".bin",
                         model_file_type);
}
...
void GlmDecoderLayerWeight<T>::mallocWeights()
{
    ...
    deviceMalloc(&weights_ptr[2], hidden_units_ * hidden_units_ / tensor_para_size_);
