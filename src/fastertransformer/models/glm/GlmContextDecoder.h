...
template<typename T>
class GlmContextDecoder: public BaseLayer {
private:
    // buffer handling
...
public:
    GlmContextDecoder(size_t max_batch_size,
                       size_t max_seq_len,
                       size_t head_num,
    ...
    GlmContextDecoder(GlmContextDecoder<T> const& decoder);

    ~GlmContextDecoder();

    void forward(std::vector<Tensor>* output_tensors,
                 const std::vector<Tensor>* input_tensors,
                 const std::vector<GlmDecoderLayerWeight<T>>* decoder_layer_weights);
